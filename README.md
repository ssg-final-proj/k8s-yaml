# Stockstalk – Kubernetes Manifests

Stockstalk 프로젝트의 Kubernetes 매니페스트를 관리하는 저장소입니다.  
...
- 애플리케이션 서비스는 `vss` 네임스페이스에 배포
- Redis / Kafka는 `infra` 네임스페이스에 배포
- 외부 요청은 ALB Ingress를 통해 각 서비스로 라우팅
- 공통 설정값은 ConfigMap / ExternalSecret 기반으로 주입
...
- `vss` : 애플리케이션 서비스
- `infra` : Redis, Kafka 등 공용 인프라 컴포넌트
...
Secrets Manager 접근을 위해 `secrets-sa` ServiceAccount를 사용합니다.
...
Kafka Pod가 Ready 상태가 되기까지 시간이 오래 걸릴 수 있습니다.  
...
- 운영 환경에서는 ConfigMap에 포함된 민감 정보는 Secret 또는 AWS Secrets Manager로 분리하는 방식을 권장합니다.
